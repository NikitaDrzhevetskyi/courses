      ...
      <tr>
          <td>
              <select  class="courseInfo form-control" id="Page">
                  <?php for ($i = 0; $i < $courses['count']['count']/15; $i++){ ?>
                      <?php if ($i === 0){ ?>
          ...
          <td><input type="text" class="form-control courseInfo" id="CourseName"></td>
          <td>
              <select  class="form-control courseInfo" id="CourseLanguage">
              <option value="" selected></option>
              <?php foreach ($params['languages'] as $el) { ?>
                  <option value="<?php echo $el['id']; ?>"><?php echo $el['name']; ?></option>
          <?php } ?>
          </td>
          <td><select class="form-control courseInfo" id="CourseLevel">
                  <option value="" selected></option>
                  <?php foreach ($params['levels'] as $el) { ?>
                  <option value="<?php echo $el['id']; ?>"><?php echo $el['code']; ?></option>
              <?php } ?>
          </td>
          <td><select class="form-control courseInfo" id="CourseCategory">
                  <option value="" selected></option>
                  <?php foreach ($params['categories'] as $el) { ?>
                  <option value="<?php echo $el['id']; ?>"><?php echo $el['name']; ?></option>
              <?php } ?>
          </td>
          <td><input type="text" class="form-control courseInfo" id="CourseLessons"></td>
          <td><input type="text" class="form-control courseInfo" id="CoursePrice"></td>
          <td><input type="date" class="form-control courseInfo" id="CourseStart"></td>
          <td><input type="date" class="form-control courseInfo" id="CourseFinish"></td>

          <td><select class="form-control courseInfo" id="CourseTypeActive" name="course" >
                  <option value="" selected></option>
        ...
        <tr class="text-center" >
                ...
                <td><a href="/admin/courseinfo?id=<?php echo $el['id']; ?>">Оновити</a></td>
                <td><a href=" /admin/delcourse?id=<?php echo $el['id']; ?>">Видалити</a></td>
            ...
            $.ajax({
                ...
                success: function (item) {
                    ...
                    for (var i = 0; i < Object.keys(courses['course']).length; i++) {
                        ...
                        $("#tbody").append("<tr" + " class='tr text-center' id='" + courses['course'][i]['id'] + "'><td></td><td>" + courses['course'][i]['name'] + "</td><td>" + courses['course'][i]['language'] + "</td><td>" + courses['course'][i]['level']
                            ...
                            + "</td><td>" + etap +
                            "</td><td><a href='/admin/courseinfo?id=" + courses['course'][i]['id'] + "'>Оновити</a></td><td><a href='/admin/delcourse?id=" + courses['course'][i]['id'] + "'>Видалити</a></td><tr>")
                    }

                    $("#Page").children().remove();
                    for (var j = 0; j < courses['count']['count'] / 15; j++) {
                        var tmp = j + 1;
                        if (tmp == courses['page']) {
